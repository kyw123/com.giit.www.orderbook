...
public class AuthFilter extends AuthenticatingFilter {
    ...
    /**
     * 步骤2，拒绝访问的请求，会调用onAccessDenied方法，onAccessDenied方法先获取 token，
     * 再调用executeLogin方法
     * 拦截之后进入AuthFilter的onAccessDenied()，这里获取token后判断token是否isBlank
     * 。如果是，代表请求未携带token，直接默认返回400，未登录给前端，
     * 流程就结束了。如果携带了token则进入第三步，继续流程。
     * @param request
     * @param response
     * @return
     * @throws Exception
